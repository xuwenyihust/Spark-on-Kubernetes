...
class SparkApp:
  ...
  @staticmethod
  def create_spark_session(notebook_path: str = None):
    logger.info(f"Creating spark session for notebook path: {notebook_path}")
    ...
    try:
        ...
        # The actual Spark app ID will be generated by Spark itself
        # when create_spark() is called in the notebook
        # We'll store it when we see it in the notebook output
